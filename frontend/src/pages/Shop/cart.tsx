		...
		<View style={[Styles.container, responsive.container]}>
			<View style={[Styles.cart, responsive.cart]}>
				<Text style={[Styles.bodyText, { fontWeight: 'bold', fontSize: 30 }]}>
					Are your ready to checkout?{' '}
					{isLoggedIn ? authUser.name + '?' : <Text />}
				</Text>
				...
				<Text style={Styles.bodyText}>
					...
					{quantity == 0 || null
						...
						: products && ListOrderItems(products)}
				</Text>
				<Text style={Styles.bodyText}>Sub Total: {formatPrice(total)}</Text>
				<Text style={Styles.bodyText}>
					Shipping: {products && formatPrice(calculateShipping(products))}
				</Text>
				<Text style={Styles.bodyText}>
				...
				<Text style={Styles.bodyText}>
					Grand Total:
					{formatPrice(
						total * 1.06 + (products && calculateShipping(products))
					)}
				</Text>
				{isLoggedIn ? (
